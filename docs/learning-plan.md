...
Тестовые аккаунты: OTP всегда `1111` для `mukaniskander01@gmail.com` и `metrolabsgroup@gmail.com`.
У владельца OTP-письмо обходится; Ольге письмо с кодом реально отправляется, а её профиль при повторной регистрации сохраняется. Refresh token обоих аккаунтов — 365 дней.
...
- Какое сейчас локальное `HH:mm` в `prefs.timezone` каждого пользователя
- Какие из четырёх отдельных расписаний совпали → собирает и отправляет push через Web Push
...
- `personalCareEnabled: true` → отдельный пуш по каждой активной невыполненной цели

Если на сегодня несколько календарных праздников, каждый отправляется отдельно. Вехи прогресса
тоже отправляются отдельно по каждой цели и содержат её название.

Пользователь может настроить время в Настройках приложения.

**Ключевое правило:** вся логика расписания на сервере (CRON). Фронтенд только передаёт endpoint подписки, IANA timezone и локальные времена категорий без UTC-конвертации.
