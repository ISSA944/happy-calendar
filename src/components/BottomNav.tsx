...
type AppTabPath = '/home' | '/bookmarks' | '/settings'

type BottomNavProps = {
  onTabIntent?: (path: AppTabPath) => void
}

export function BottomNav({ onTabIntent }: BottomNavProps) {
    ...
    <nav className="mx-auto w-full max-w-[430px] landscape:max-w-[860px] z-50 bg-white flex-shrink-0">
      ...
      <div className="rounded-t-[2rem] shadow-[0_-4px_24px_rgba(0,0,0,0.06)] border-t border-stone-100/80">
        <div className="flex justify-around items-center px-4 pt-3 pb-3">
          {/* Home */}
          <NavLink to="/home" onPointerDown={() => onTabIntent?.('/home')} className={({ isActive }) => tabClass(isActive)}>
          ...
          {/* Bookmarks */}
          <NavLink to="/bookmarks" onPointerDown={() => onTabIntent?.('/bookmarks')} className={({ isActive }) => tabClass(isActive)}>
            <motion.span whileTap={{ scale: 0.92 }} className="flex flex-col items-center">
          ...
          {/* Settings */}
          <NavLink to="/settings" onPointerDown={() => onTabIntent?.('/settings')} className={({ isActive }) => tabClass(isActive)}>
            <motion.span whileTap={{ scale: 0.92 }} className="flex flex-col items-center">
