...
export class AiService {
  ...
  > = {
    'Овен ♈︎': {
      ...
      detailed:
        'Марс активно влияет на ваш знак, придавая мощную внутреннюю силу. Не бойтесь брать инициативу.',
    ...
    'Телец ♉︎': {
      ...
      detailed:
        'Венера благоволит стабильности. Сегодня хороший день для долгосрочных инвестиций и планирования.',
    ...
    'Близнецы ♊︎': {
      ...
      detailed:
        'Меркурий усиливает ваше красноречие. Используйте слова как мост к новым возможностям.',
  ...
  > = {
    '01.01': {
      name: 'Новый год',
      description: 'С Новым годом! Пусть этот год принесёт счастье.',
      icon: 'celebration',
    },
    '08.03': {
      name: 'Международный женский день',
      description: 'Поздравляем всех женщин!',
      icon: 'favorite',
    },
    '14.02': {
      name: 'День святого Валентина',
      description: 'День любви и нежности.',
      icon: 'favorite',
    },
    '09.05': {
      name: 'День Победы',
      description: 'Помним и гордимся.',
      icon: 'military_tech',
    },
    '31.12': {
      name: 'Канун Нового года',
      description: 'Время подводить итоги и мечтать!',
      icon: 'celebration',
    },
  };
  ...
  async generateDailyPack(
    ...
    this.logger.log(
      `[MOCK] generateDailyPack for user=${userId}, sign=${context.zodiacSign}, mood=${context.mood}`,
    );
    ...
    const horoscope = this.horoscopes[context.zodiacSign] ?? {
      ...
      detailed:
        'Планеты выстраиваются в гармоничную конфигурацию. Это отличное время для самоанализа.',
  ...
  async updateMoodSupport(
    ...
    this.logger.log(
      `[MOCK] updateMoodSupport for user=${userId}, mood=${newMood}`,
    );
