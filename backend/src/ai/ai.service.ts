...
export class AiService {
          ...
          messages: [
            ...
            },
            {
              ...
              content:
                ...
                `- horoscope: главный прогноз на день для ${context.zodiacSign} (2–3 предложения). ` +
                'Отражай типичные черты знака, говори конкретно о сегодняшнем дне, вдохновляющий тон.\n\n' +
                `- horoscopeDetailed: глубокий астрологический анализ для ${context.zodiacSign} (3–4 предложения). ` +
                'Упомяни планеты-покровители знака, лунную энергию, как это влияет именно на этот знак сегодня.\n\n' +
                '- advice: конкретный практический совет на день (1–2 предложения). ' +
  ...
  async updateMoodSupport(
    ...
    this.logger.log(`updateMoodSupport user=${userId}, mood=${newMood}, sign=${zodiacSign ?? 'unknown'}, name=${name ?? 'unknown'}, gender=${gender ?? 'unknown'}`);

    if (this.openai) {
      try {
        const signContext = zodiacSign ? `Знак зодиака: ${zodiacSign}. ` : '';
        const nameContext = name ? `Имя пользователя: ${name}. ` : '';
        const genderContext = gender === 'M' ? 'Пол: мужской. ' : gender === 'F' ? 'Пол: женский. ' : '';
  ...
  async generateSupportPhrasesBatch(
    ...
    this.logger.log(`generateSupportPhrasesBatch mood=${mood}, sign=${zodiacSign ?? 'unknown'}, name=${name ?? 'unknown'}, gender=${gender ?? 'unknown'}`);
