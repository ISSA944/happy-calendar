...
export class AiService {
  ...
  async generateDailyPack(
            ...
            messages: [
              ...
              },
              {
                ...
                content:
                  ...
                  `- horoscope: содержательный главный прогноз на день для ${context.zodiacSign}: строго 3–4 предложения и 240–650 символов. ` +
                  'Отражай типичные черты знака, говори конкретно о сегодняшнем дне и избегай общих фраз.\n\n' +
                  `- horoscopeDetailed: глубокий астрологический анализ для ${context.zodiacSign}: строго 6–8 предложений и 520–1400 символов. ` +
                  'Упомяни планеты-покровители знака, лунную энергию, как это влияет именно на этот знак сегодня.\n\n' +
                  '- advice: строго 2 содержательных предложения с конкретным практическим советом. Должен быть действенным, не абстрактным.\n\n' +
                  ...
                  '(строго 3–4 предложения и 220–650 символов). ' +
                  (context.name
    ...
    return {
      horoscope: stripHtml(
        `${horoscope.main} Выбери одно направление, где твоя решительность принесёт ощутимый результат, и не распыляй внимание на чужие ожидания. Вечером отметь даже небольшой прогресс: сегодня устойчивый темп важнее эффектного рывка.`,
      ),
      horoscopeDetailed: stripHtml(
        `${horoscope.detailed} Лунная энергия делает внутренние реакции заметнее, поэтому перед важным решением полезно проверить, чего ты хочешь на самом деле. В делах лучше опираться на последовательность: одна завершённая задача даст больше уверенности, чем несколько громких начинаний. В общении прямота будет услышана, если оставить собеседнику пространство для ответа и не торопить его. К вечеру стоит снизить темп, восстановить силы и спокойно выбрать главный ориентир на завтра.`,
      ),
      advice: stripHtml(
        `${horoscope.advice} Запиши один конкретный шаг, который реально закончить сегодня, и начни именно с него.`,
      ),
      moon: stripHtml(horoscope.moon),
      aspect: stripHtml(horoscope.aspect),
      holiday: holiday.name,
      supportPhrase: stripHtml(
        `${phrases[Math.floor(Math.random() * phrases.length)]} Сейчас не нужно доказывать, что ты справляешься идеально: достаточно услышать себя и выбрать самый бережный следующий шаг. То, что сегодня кажется тяжёлым, станет понятнее, когда ты вернёшь себе немного тишины, времени и опоры.`,
      ),
      isFallback: true,
    };
  }
            ...
            messages: [
              ...
              },
              {
                ...
                content:
                  ...
                  'Напиши одну содержательную персональную фразу поддержки: строго 3–4 предложения и 220–650 символов. ' +
  ...
  private expandSupportFallback(base: string): string {
    return stripHtml(
      `${base} Сейчас не нужно доказывать, что ты справляешься идеально: достаточно услышать себя и выбрать самый бережный следующий шаг. То, что сегодня кажется тяжёлым, станет понятнее, когда ты вернёшь себе немного тишины, времени и опоры.`,
    );
  }
            ...
            messages: [
              ...
              },
              {
                ...
                content:
                  ...
                  'Напиши 5 разных персональных фраз поддержки: каждая строго 3–4 предложения и 220–650 символов. ' +
