...
export class AiService {
  ...
  async updateMoodSupport(
    ...
    this.logger.log(
      `updateMoodSupport user=${userId}, mood=${newMood}, sign=${zodiacSign ?? 'unknown'}, name=${name ?? 'unknown'}, gender=${gender ?? 'unknown'}`,
    );
  ...
  async generateSupportPhrasesBatch(
    ...
    this.logger.log(
      `generateSupportPhrasesBatch mood=${mood}, sign=${zodiacSign ?? 'unknown'}, name=${name ?? 'unknown'}, gender=${gender ?? 'unknown'}`,
    );
    ...
    if (this.openai) {
      try {
        ...
        const batch = [parsed.p1, parsed.p2, parsed.p3, parsed.p4, parsed.p5]
          ...
          .filter(Boolean);
        this.logger.log(
          `generateSupportPhrasesBatch OK, got ${batch.length} phrases`,
        );
        return batch;
      } catch (err) {
        this.logger.error(
          'generateSupportPhrasesBatch failed — using fallback',
          (err as Error).message,
        );
      }
    }
